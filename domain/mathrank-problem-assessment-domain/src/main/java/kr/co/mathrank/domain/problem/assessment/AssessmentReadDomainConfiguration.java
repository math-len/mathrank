...
public class AssessmentReadDomainConfiguration {
	...
	RequiredCacheSpec assessmentReadDomainAssessmentPageCacheSpec() {
		return createCacheSpec(ASSESSMENT_READ_PAGE_CAHCE, Duration.ofSeconds(60));
	}

	@Bean
	RequiredCacheSpec assessmentReadDomainAssessmentRankCacheSpec() {
		return createCacheSpec(ASSESSMENT_READ_RANK_CACHE, Duration.ofSeconds(60));
	}
	@Bean
	RequiredCacheSpec assessmentReadDomainAssessmentSolutionCacheSpec() {
		return createCacheSpec(ASSESSMENT_READ_SOLUTION_CACHE, Duration.ofSeconds(60));
	}
	@Bean
	RequiredCacheSpec assessmentReadDomainAssessmentStatisticsCacheSpec() {
		return createCacheSpec(ASSESSMENT_STATISTICS_CACHE, Duration.ofSeconds(60));
	}

	@Bean
	RequiredCacheSpec assessmentReadDomainAssessmentSingleSubmissionCacheSpec() {
		return createCacheSpec(ASSESSMENT_SUBMISSION_SINGLE_CACHE, Duration.ofSeconds(60));
	}
